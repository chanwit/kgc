...
func (p *parser) parseStructType() *ast.StructType {
    ...
    var pos token.Pos
    var isCaseStruct bool = false
    if p.tok == token.CASESTRUCT {
        pos = p.expect(token.CASESTRUCT)
        isCaseStruct = true
    } else {
	    pos = p.expect(token.STRUCT)
    }

    var traitExpr ast.Expr
    if p.tok == token.BORROWS {
        p.expect(token.BORROWS)
        traitExpr = p.parseTypeName()
    }
	...
	return &ast.StructType{pos, &ast.FieldList{lbrace, list, rbrace}, false, isCaseStruct, traitExpr}
}
...
func (p *parser) parseAnonymousFuncType() *ast.FuncType {
	if p.trace {
		defer un(trace(p, "AnonymousFuncType"))
	}
	pos := p.expect(token.LBRACE)
	params, results := p.parseAnonymousSignature()
	return &ast.FuncType{pos, params, results}
}
...
// two cases:
// 1. func(it interface{}) ==> {      }
// 2. func(a interface{})  ==> { a -> }
// 3. func()               ==> { ->   }
//
func (p *parser) parseAnonymousSignature() (params, results *ast.FieldList) {
	// list, typ := p.parseVarList(false)

	results = &ast.FieldList{}
	p.expect(token.FWARROW)
...
func (p *parser) parseInterfaceType() *ast.InterfaceType {
	...
	return &ast.InterfaceType{pos, &ast.FieldList{lbrace, list, rbrace}, false, isTrait}
}
...
func (p *parser) tryRawType(ellipsisOk bool) ast.Expr {
	...
	case token.STRUCT, token.CASESTRUCT:
...
/*
func (p *parser) parseAnonymousFuncLit() ast.Expr {
	if p.trace {
		defer un(trace(p, "AnonymousFuncLit"))
	}
	// typ := p.parseAnonymousFuncType()
	p.exprLev++
	body := p.parseAnonymousBody()
	p.exprLev--
	return &ast.FuncLit{typ, body}
}
*/
...
func (p *parser) parseOperand() ast.Expr {
	...
	//case token.LBRACE:
	//	return p.parseAnonymousFuncLit()
...
func (p *parser) parseMatch(x ast.Expr) ast.Expr {
	return nil
}
...
L:
	for {
		switch p.tok {
		case token.MATCH:
			x = p.parseMatch(p.checkExprOrType(x))
		case token.PERIOD:
...
func (p *parser) parseSwitchStmt() ast.Stmt {
	...
	if isExprSwitch(s2) {
		...
		p.expectSemi()
		return &ast.SwitchStmt{pos, s1, p.makeExpr(s2), body, isMatch}
	}
...
func (p *parser) parseFuncDecl() *ast.FuncDecl {
        ...
        &ast.FieldList {
          List: []*ast.Field {
            &ast.Field {
              ...
              Type: &ast.StarExpr {
                X: &ast.Ident {
                  Name: "_" + ident.Name + "Impl" } },
              Tag: nil,
              Comment: nil } } }
        */
        n := ident.Name
        ident = p.parseIdent()
        ident.Name = "_" + n + "Impl_" + ident.Name
	...
	params, results := p.parseSignature()
    if p0 != nil {
        p := make([]*ast.Field, len(params.List)+1)
        p[0] = p0
