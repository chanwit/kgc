...
func TestSemis(t *testing.T) {
	for _, line := range lines {
		checkSemi(t, line, scanner.AllowIllegalChars|scanner.InsertSemis)
		checkSemi(t, line, scanner.AllowIllegalChars|scanner.InsertSemis|scanner.ScanComments)

		// if the input ended in newlines, the input must tokenize the
		// same with or without those newlines
		for i := len(line) - 1; i >= 0 && line[i] == '\n'; i-- {
			checkSemi(t, line[0:i], scanner.AllowIllegalChars|scanner.InsertSemis)
			checkSemi(t, line[0:i], scanner.AllowIllegalChars|scanner.InsertSemis|scanner.ScanComments)
		}
	}
}
